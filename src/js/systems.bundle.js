(() => {
  ...
  var ACCENT_PALETTE = ["#6366f1", "#a70d2a", "#f59e0b", "#3b82f6", "#ec4899", "#14b8a6", "#8b5cf6", "#f97316", "#06b6d4", "#84cc16"];
  ...
  var SystemMaker = (() => {
    ...
    function normalizeData(input, entities, previousSpec) {
      ...
      Object.values(entities).forEach((entity) => {
        ...
        data[entity.id] = Array.isArray(rows) && rows.length ? rows.map((r, idx) => normalizeRecord(r, entity, idx)) : generateRows(entity, entity.id);
      });
      return data;
    }
...
${String(rawText || JSON.stringify(parsed || {})).slice(0, 18e3)}` }
    ...
    function sortIcon(f) {
      ...
      return sortState.dir === "asc" ? `<svg viewBox="0 0 10 14" fill="none" stroke="currentColor" stroke-width="1.6" width="9" height="12"><path d="M5 2v10M2 5l3-3 3 3"/></svg>` : `<svg viewBox="0 0 10 14" fill="none" stroke="currentColor" stroke-width="1.6" width="9" height="12"><path d="M5 2v10M2 9l3 3 3-3"/></svg>`;
    }
    ...
    function handleImportFile(file, entity) {
      ...
      reader.onload = (e) => {
        ...
        $("sysImportBody").querySelectorAll("select[data-col-idx]").forEach((sel) => {
          sel.onchange = () => {
            importState.mappings[Number(sel.dataset.colIdx)] = sel.value || void 0;
          };
        });
      };
