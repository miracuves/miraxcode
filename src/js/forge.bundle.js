(() => {
  ...
  // src/js/modes/forge/plans-templates.js
  function createForgePlansTemplatesApi(ctx) {
    const {
      log,
      renderableNodes,
      isKnifeLikePrompt,
    ...
    function fallbackPlan(prompt) {
      ...
      if (/iphone|phone|smartphone|mobile/.test(q)) return phonePlan2(prompt);
      if (/laptop|macbook|notebook|computer/.test(q)) return laptopPlan2(prompt);
    ...
    function electronicsDeskScenePlan(prompt) {
      ...
      nodes.push(...prefixNodes(laptopPlan2(prompt).nodes.filter((node) => node.role !== "audit"), "laptop", [0.28, 0.37, -0.1]));
      if (/iphone|phone|smartphone|mobile/.test(q)) {
        nodes.push(...prefixNodes(phonePlan2(prompt).nodes.filter((node) => node.role !== "audit"), "phone", [-0.82, 0.36, 0.32], [0, 0.02, -0.35]));
      }
    ...
    return {
      ...
      tablePlan,
      personPlan,
      phonePlan: phonePlan2,
      laptopPlan: laptopPlan2,
      electronicsDeskScenePlan,
      genericPlan,
      humanBodyModelNodes,
      humanSkeletonLibraryNodes,
      offsetNodes,
      prefixNodes,
      ...
      coneMesh,
      tubeMesh,
      makeConcaveOvalBowlMeshParams,
      makeTaperedHandleMeshParams,
      makeEllipsoidMeshParams,
      makeConeMeshParams,
      makeTubeMeshParams,
      normalize3,
      cross3
